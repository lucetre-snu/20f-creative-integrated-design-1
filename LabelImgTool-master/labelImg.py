...
from libs.maskInfoManager import MaskInfoManager
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, filename=None):
        ...
        # mask file IO manager
        self.maskInfoManager = MaskInfoManager()
        ...
        self.brush_eraser = QCheckBox('Brush Eraser')
        self.brush_eraser.setTristate(False)
        self.brush_eraser.stateChanged.connect(self.set_brush_eraser)
        brush_layout.addWidget(self.brush_eraser)
        self.brush_clear = QPushButton('Brush Clear')
        self.brush_clear.clicked.connect(self.set_brush_clear)
        brush_layout.addWidget(self.brush_clear)
        self.brush_dock = QDockWidget('Brush Tools',self)
        self.brush_dock.setObjectName('Brush')
        ...
        self.labelSelectDock = QDockWidget('Select Label', self)
        self.labelSelectDock.setObjectName('selectLabel')
        ...
        self.label_color_dock = QDockWidget('Label Color Map', self)
        self.label_color_dock.setObjectName('label_color')
        ...
        remote_settings = action('&Remote DB Settings', self.setRemoteUrl,
                                 'Ctrl+m', 'set remote url')
        settings = action('Settings', self.setSettings, 'Ctrl+t', 'settings')
        ...
        openAnnotation = action('&Open Annotation', self.openAnnotation,
                                'Ctrl+q', 'openAnnotation', 'Open Annotation')

        openNextImg = action('&Next Image', self.openNextImg,
                             'Right', 'next', 'Open Next')
        ...
        color2 = action('Box &Fill Color', self.chooseColor2,
                        'Ctrl+Shift+L', 'color', 'Choose Box fill color')
        ...
        createRect = action('Create\nRectBox', self.createRect,
                        'Ctrl+N', 'new', 'Draw a new Box', enabled=False)
        ...
        copy = action(
            ...
            'Ctrl+D',
            'copy',
            'Create a duplicate of the selected Box',
            enabled=False)
        ...
        hideAll = action('&Hide\nShape', partial(self.togglePolygons, False),
                         'Ctrl+H', 'hide', 'Hide all Boxs',
                         enabled=False)
        showAll = action('&Show\nShape', partial(self.togglePolygons, True),
                         'Ctrl+A', 'hide', 'Show all Boxs',
                         enabled=False)
        ...
        edit = action(
            ...
            'Ctrl+E',
            'edit',
            'Modify the label of the selected Box',
            enabled=False)
    ...
    def set_brush_eraser(self,value):
        if value == Qt.Checked:
            self.canvas.setBrushCol(0)
        else:
            self.canvas.setBrushCol(1)
    ...
    def newShape(self):
        ...
        if text is not None:
            ...
            if self.enable_instance_seg:
                ...
                msg = 'Is it a new instance with id '+str(self.current_instance_id+1)
                if yes == QMessageBox.question(self, 'Attention', msg, yes | no):
    ...
    def openFile(self, _value=False):
        ...
        numLayer, iBuf = self.maskInfoManager.loadMaskFile(filename, self.image_size, self.canvas.baseBrushCol, self.canvas.eraseBrushCol)
        if (numLayer == self.canvas.MAX_MASK_LAYER) and iBuf :
            self.canvas.mask_pixmap = iBuf
    ...
    def saveFile(self, _value=False):
        assert not self.image.isNull(), "cannot save empty image"
        self.maskInfoManager.saveMaskFile(self.filename, self.image_size, self.canvas.MAX_MASK_LAYER, self.canvas.mask_pixmap)
    ...
    def deleteSelectedShape(self):
        yes, no = QMessageBox.Yes, QMessageBox.No
        msg = 'You are about to permanently delete this Box, proceed anyway?'
    ...
    def load_label_color_map(self):
        ...
        if os.path.exists(self.label_color_map_path):
            ...
            self.label_color_map = [color+[50] for color in list(COLORMAP.values())]
    ...
    def newOnkeyPressEvent(self, e):
        ...
        if key == Qt.Key_Escape and self.canvas.current:
            print('ESC press')
            self.canvas.current = None
            self.canvas.drawingPolygon.emit(False)
            self.canvas.update()
        elif key == Qt.Key_Return and self.canvas.canCloseShape():
            self.canvas.finalise()
        elif key == Qt.Key_1 :
            self.canvas.curMaskIdx = 0
            self.canvas.repaint()
        elif key == Qt.Key_2 :
            self.canvas.curMaskIdx = 1
            self.canvas.repaint()
            ...
            self.brush_size_sl_ctl(-5)
        elif key == Qt.Key_W :
            self.brush_size_sl_ctl(-1)
        elif key == Qt.Key_E :
            self.brush_size_sl_ctl(1)
        elif key == Qt.Key_R :
            self.brush_size_sl_ctl(5)
        elif key == Qt.Key_T :
            if not self.brush_eraser.isChecked() :
                self.brush_eraser.setChecked(True)
            else :
                self.brush_eraser.setCheckState(False)
            


    def brush_size_sl_ctl(self, adj):
        curVal = self.brush_size_sl.value()
        curVal += adj
        if (curVal<1) :
            curVal = 1
        if (curVal>100) :
            curVal = 100
        self.brush_size_sl.setValue(curVal)
