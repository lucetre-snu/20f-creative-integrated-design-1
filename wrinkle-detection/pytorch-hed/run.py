...
for strOption, strArgument in getopt.getopt(sys.argv[1:], '', [ strParameter[2:] + '=' for strParameter in sys.argv[1::2] ])[0]:
	...
	if strOption == '--in' and strArgument != '': in_dir = strArgument # path to the input image
# end
...
if __name__ == '__main__':
	import os
	for filename in os.listdir(os.path.join(os.getcwd(), in_dir)):
		if filename.endswith('jpg'):
			print(filename)
			in_file = os.path.join(os.getcwd(), in_dir+filename)
			out_file = os.path.join(os.getcwd(), in_dir+os.path.splitext(filename)[0]+'_out.png')

			tenInput = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open(in_file))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))

			tenOutput = estimate(tenInput)

			PIL.Image.fromarray((tenOutput.clamp(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, 0] * 255.0).astype(numpy.uint8)).save(out_file)
		# end
