import numpy as np

import time
import torch
import torch.nn as nn
from torch.utils.tensorboard import SummaryWriter
import matplotlib.pyplot as plt

from unet.model import UNet
from unet.dataset import *
from unet.util import *
from unet.loss import *


def train(model, loader_train, loss_function, num_batch, current_epoch, writer_train, device, optim):
    start = time.time()
    model.train()
    loss_arr = []
    acc_arr = []
    tp_arr = []
    tn_arr = []
    fp_arr = []
    fn_arr = []
    f1_arr = []

    for batch, data in enumerate(loader_train, 1):
        label = data['label'].to(device)
        input = data['input'].to(device)

        pred = model(input)

        optim.zero_grad()

        loss = loss_function(pred, label)
        loss.backward()

        optim.step()

        loss_arr += [loss.item()]

        tp, tn, fp, fn, f1 = f1_score(pred, label)
        tp_arr += [tp.item()]
        tn_arr += [tn.item()]
        fp_arr += [fp.item()]
        fn_arr += [fn.item()]
        f1_arr += [f1.item()]

        label = fn_tonumpy(label)
        pred = fn_tonumpy(fn_class(pred))

        B, C, H, W = label.shape
        acc = np.sum(label == pred) / (B * C * H * W)
        acc_arr += [acc]

        print("TRAIN: EPOCH %04d | BATCH %04d / %04d | LOSS %.4f | ACC %.4f | F1 SCORE %.4f | TIME %.4f" %
            (current_epoch, batch, num_batch, loss.item(), acc, f1.item(), time.time()-start))
        writer_train.add_scalar('loss_step', loss.item(), num_batch * current_epoch + batch)

        """
        debugging code
        input = fn_tonumpy(fn_denorm(input, mean=0.5, std=0.5))
        for j in range(label.shape[0]):
            id = j
            plt.imsave(os.path.join("./temp/", 'png', 'label_%04d_%04d.png' % (batch, id)), label[j].squeeze(), cmap='gray')
            plt.imsave(os.path.join("./temp/", 'png', 'input_%04d_%04d.png' % (batch, id)), input[j].squeeze(), cmap='gray')
            plt.imsave(os.path.join("./temp/", 'png', 'output_%04d_%04d.png' % (batch, id)), pred[j].squeeze(), cmap='gray')
        """

    input = fn_tonumpy(fn_denorm(input, mean=0.5, std=0.5))
    if (current_epoch % 10) == 0:
        writer_train.add_image('label', label, current_epoch, dataformats='NHWC')
        writer_train.add_image('input', input, current_epoch, dataformats='NHWC')
        writer_train.add_image('output', pred, current_epoch, dataformats='NHWC')
    writer_train.add_scalar('loss', np.mean(loss_arr), current_epoch)
    writer_train.add_scalar('f1', np.mean(f1_arr), current_epoch)

    return np.mean(loss_arr), np.mean(acc_arr), np.mean(f1_arr), np.mean(tp_arr), np.mean(tn_arr), np.mean(fp_arr), np.mean(fn_arr)


